                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="px-4 py-3">
                                                            @foreach (json_decode($b->name_bill, true) as $id_course)
                                                                @php
                                                                    ...
                                                                    $tuitionFeeInUSD = \App\Models\Course::getTuitionFeeInUSD(
                                                                        $id_course,
                                                                    );
                                                                @endphp
                                                        ...
                                                        <td class="px-4 py-3">
                                                            <div class="flex items-center">
                                                                <img src="images/checkbox.png" class="h-7 w-7"
                                                                    alt="">
                                                            </div>
                                                        </td>
                                                    </tr>
                            ...
                            @else
                                <div class="p-6 text-gray-900">
                                    <div class="flex flex-col w-full mb-20">
                                        <div
                                            class="bg-white border rounded-lg shadow-lg px-6 py-8 max-w-md mx-auto mt-8">
                                            <h1 class="font-bold text-2xl my-4 text-center text-blue-600">TuitionFee
                                            </h1>
                                            <hr class="mb-2">
                                            <div class="flex justify-between mb-6">
                                                <h1 class="text-lg font-bold">Invoice #: {{ $b->id_bill }}</h1>
                                                <div class="text-gray-700 ml-12">
                                                    <div>Date: {{ $b->created_at }}</div>
                                                </div>
                                            </div>
                                            <div class="mb-8">
                                                <h2 class="text-lg font-bold mb-4">Bill To:</h2>
                                                <div class="text-gray-700 mb-2">Tên: {{ Auth::user()->name }}</div>
                                                <div class="text-gray-700 mb-2">Địa chỉ: {{ Auth::user()->address }}</div>
                                                <div class="text-gray-700 mb-2">Email: {{ Auth::user()->email }}</div>
                                                <div class="text-gray-700">Số điện thoại: {{ Auth::user()->phone }}</div>
                                            </div>
                                                    ...
                                                    @foreach (json_decode($b->name_bill, true) as $id_course)
                                                        ...
                                                        <tr>
                                                            <td class="text-left text-gray-700">
                                                                {{ $course->id_course ?? 'N/A' }}</td>
                                                            <td class="text-right text-gray-700">
                                                                ${{ number_format($tuitionFeeInUSD, 2) }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <td class="text-right font-bold text-gray-700">
                                                            ${{ number_format($b->tuitionFee / 23000 ?? 0, 2) }}</td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                                <form class="flex justify-center items-center"
                                                    action="{{ route('paypal', ['id' => $b->id_bill, 'date' => date('d-m-Y')]) }}"
                                                    method="post">
                                                    @csrf
                                                    <input type="hidden" name="price"
                                                        value="{{ number_format($b->tuitionFee / 23000 ?? 0, 2) }}">
                                                    <button
                                                        class="transform transition-transform duration-400 hover:scale-125 "
                                                        type="submit" style="background: none; border: none;">
